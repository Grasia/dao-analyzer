"""
   Descp: Strategy pattern to create total token holders metric.
...
class StTotalTokenHolders(IMetricStrategy):
    ...
    def process_data(self, df: pd.DataFrame) -> StackedSerie:
        ...
        # Cumulative sum (total holders)
        df = df.set_index(self.__DF_DATE).sort_index().cumsum().reset_index()
