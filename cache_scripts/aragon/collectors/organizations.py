...
def _request_organizations(last_id: str, endpoint: str) -> List[Dict]:
    requester: ApiRequester = ApiRequester(endpoint=endpoint)
    print("Requesting Organization\'s data ...")
    start: datetime = datetime.now()

    orgs: List[Dict] = requester.n_requests(query=ORGANIZATION_QUERY, last_id=last_id, 
        result_key=META_KEY)
...
def _apply_names(df: pd.DataFrame, net: str):
    if net in NAMES.keys() and not df.empty:
...
def update_organizations(meta_data: Dict, net: str, endpoints: Dict) -> None:
    ...
    meta_data[net][META_KEY]['last_id'] = orgs[-1]['id'] if orgs else ""
