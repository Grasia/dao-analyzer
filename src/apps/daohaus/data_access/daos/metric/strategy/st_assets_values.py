...
class StAssetsValues(IMetricStrategy):
    ...
    def process_data(self, df: pd.DataFrame) -> Any:
        ...
        def _valueBy(df, k=1):
            aux = df.groupby(_flatten(self.__group_path[:k]))[self.__values_cols].sum()
            return aux
        ...
        for i, level in enumerate(self.__group_path):
            ...
            dfg = _valueBy(top, i+1).reset_index()
            ...
            df_tree['customData'] = _customData(dfg)
            df_tree['value'] = _value(dfg, i+1)
        ...
        df_trees.append(pd.DataFrame({
            ...
            'parent': [''],
            'value': [0],
            'customData': [_customData(df[self.__values_cols].sum())],
        }))
